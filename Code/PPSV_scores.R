...
#head(db_tp)
# Read sequence of uniprot protein
seq_df <- read.table('../sequence_unp.txt', header = T, stringsAsFactors = F)
# Read string database with uniprot name from Uniprot database (.tab)
...
#nrow(string800)
g<-graph.data.frame(string800,directed = F) # generage network with 800 confident score
# Remove multiple edges
net<-simplify(g, remove.multiple=T) #net is network of string.data 800 confident score
# Read kegg_unp  
...
mainScores <- function(p1, p2) {
  ...
  # Pathway score
  func_score <- length(intersect(kegg_unp$V1[which(kegg_unp$Entry == p1)], kegg_unp$V1[which(kegg_unp$Entry == p2)]))
